...
if(isset($_REQUEST['sso']) || isset($_REQUEST['login'])){
...
}else if(isset($_REQUEST['slo']) || isset($_REQUEST['logout'])){
	...
	$_SESSION['_login_info']['login'] = $_SESSION['_login_info']['uid'];
...
if(isset($_SESSION['ssoUserdata'])){
	echo '- You are logged in -- You have the following attributes from $_SESSION[\'_login_info\']:<br/>';
	echo "<div style='margin-left: 40px;'>";
	if(!empty($_SESSION['_login_info'])){
		echo '<table style="padding-top: 5px;"><thead><th>Name</th><th>Values</th></thead><tbody>';
		foreach($_SESSION['_login_info'] as $attributeName => $attributeValue){
...
?>
  <br/>
- <a href="?login&redirect=">Login</a> <i>(Url: <?php echo $REQUEST_SCHEME.'://'.$sso_settings['sp']['entityId']."/sso/?login&redirect=" ?>)</i><br/><br/>
- <a href="?logout&redirect=">Logout</a> <i>(Url: <?php echo $REQUEST_SCHEME.'://'.$sso_settings['sp']['entityId']."/sso/?logout&redirect=" ?>)</i><br/>
